...
export function drawBlocks(ctx: CanvasRenderingContext2D, blocks: Array<Block>, squareSize: number) {    

    for (const block of blocks) {
        const adjustedX = block.x * squareSize;
        const adjustedY = block.y * squareSize;
